function secretData(text) {
    text = text.join('\n');
    let namePattern = /\*\b[A-Z][A-Za-z]*(?=\s|$)\b/g;
    let phonePattern = /\+[0-9-]{10}(?=\s|$)/g;
    let idPattern = /![A-Za-z0-9]+(?=\s|$)/g;
    let basePattern = /_[A-Za-z0-9]+(?=\s|$)/g;

    text = text.replace(namePattern, (match) => '|'.repeat(match.length))
        .replace(phonePattern, (match) => '|'.repeat(match.length))
        .replace(idPattern, (match) => '|'.repeat(match.length))
        .replace(basePattern, (match) => '|'.repeat(match.length));
